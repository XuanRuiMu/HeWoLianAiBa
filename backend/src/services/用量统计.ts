...
export async function jiLuShiYongLiang(jiLu: ShiYongLiangJiLu): Promise<void> {
  ...
  try {
    await 数据库.query(
      `INSERT INTO "LLM用量" ("日期", "模型类型", "模型", "次数", "输入Token", "输出Token", "总Token")
       ...
       ON CONFLICT ("日期", "模型类型", "模型") DO UPDATE SET
         ...
         "更新时间" = NOW()`,
      [riQi, jiLu.moXingLeiXing, jiLu.moXing, shuRu, shuChu, zong],
    )
  } catch (cuoWu) {
...
export async function huoQuJinRiHuiZong(riQi: string = jinRiBiaoJi()): Promise<YongLiangHuiZong[]> {
  ...
  for (const [jianMing, zhi] of Object.entries(yuanShi)) {
    ...
    const xianYou = huiZong.get(chengYuan) ?? { moXingLeiXing, ciShu: 0, shuRuToken: 0, shuChuToken: 0, zongToken: 0 }
  ...
  try {
    const jieGuo = await 数据库.query(
      `SELECT "模型类型", "模型", "次数", "输入Token", "输出Token", "总Token" FROM "LLM用量" WHERE "日期" = $1 ORDER BY "总Token" DESC`,
      [riQi],
    )
