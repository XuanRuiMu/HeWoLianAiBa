...
export async function jianCeSiLianHeYi(
  ...
  const liShiWenBen = duiHuaLiShi
    .slice(-10)
    .map((liShiXiaoXi) => {
      const faSongZhe = liShiXiaoXi.fa_song_zhe_lei_xing === 'yonghu' ? '用户' : '角色'
      const meiTiMiaoShu = meiTiZhanShiWenBen(liShiXiaoXi.meiTiLeiBie, {
        yiCheHui: liShiXiaoXi.yi_che_hui,
        shiChangHaoMiao: liShiXiaoXi.meiTiShiChangHaoMiao,
        yuanShiWenJianMing: liShiXiaoXi.yuanShiWenJianMing,
      })
      return `[${liShiXiaoXi.shi_jian}] ${faSongZhe}: ${meiTiMiaoShu || liShiXiaoXi.nei_rong}`
    })
    .join('\n')
...
export async function chuLiYongHuJuJueAIHuoJieShou(
  ...
  const liShiWenBen = (dui_hua_li_shi ?? [])
    .slice(-SHENG_LI_SHI_BAI_PEI_ZHI.biaoBaiPanDuanLiShiTiaoShu)
    .map((liShiXiaoXi) => {
      const faSongZhe = liShiXiaoXi.fa_song_zhe_lei_xing === 'yonghu' ? '用户' : '角色'
      return `[${liShiXiaoXi.shi_jian}] ${faSongZhe}: ${liShiXiaoXi.nei_rong}`
    })
    .join('\n')
