...
function gouJianZhaDianTiShi(
  ...
  if (!neiRong && zhaXingTeZhi) {
    neiRong =
      `${渲染性别变体文案('fuPan', 'zhaXingJingShiDaoYu', xingBie)}\n` +
  ...
  return `${huoQuFanYi('fuPan', 'zhaXingJingShiBiaoTi')}\n${
    neiRong || 渲染性别变体文案('fuPan', 'zhaXingJingShiFallback', xingBie)
...
export async function shengChengFuPan(
  ...
  if (fuPanJiaoSeXinXi.shiFouZhaXing) {
    zongJie = `${gouJianZhaDianTiShi(jieGou, fuPanJiaoSeXinXi.xingBie, zhaXingTeZhi)}\n\n${zongJie}`
  }
