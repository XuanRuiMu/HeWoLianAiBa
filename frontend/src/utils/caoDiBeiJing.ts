...
export function gouJianCaoDiDiZhi(canShu?: CaoDiDiZhiCanShu): string {
  ...
  return `/grass-bg/grass-bg.html?v=${suoFang}&fenXi=${fenXi}&xiangSuBi=${biLi}&caiYangMiao=${encodeURIComponent(caiYang)}&zhenJianGe=${encodeURIComponent(zhenJianGe)}&chongShi=${encodeURIComponent(chongShi)}`
}
