<template>
  <div
    ...
    :class="{ 'rizhi-tuodong': 拖动中 }"
    :style="浮窗样式"
    role="dialog"
    :aria-label="huoQuFanYi('shiShiRiZhi', 'biaoTi')"
  >
    <header class="rizhi-biaoti-lan" @pointerdown="开始拖动">
...
function chuLiKuaiJieJian(shiJian: KeyboardEvent) {
  ...
  // FP-18 运行时日志流按高危能力位授权，口径与服务端 socket/日志推送.ts 的门禁同源
  if (!用户仓库.keGaoWei) return
