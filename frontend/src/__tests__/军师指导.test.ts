...
describe('FP-02 军师指导"已指导过相同聊天内容"按规则提示', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(huoQuJunShiLieBiao).mockResolvedValue(chuangJianMoNiJunShiLieBiao())
    vi.mocked(huoQuJunShiJiLu).mockResolvedValue(chuangJianMoNiJiLuLieBiao())
  ...
  it('youLiaoTianJiLu为false时显示"暂无聊天记录"提示', async () => {
    const { wrapper } = await mountJunShiZhiDao()
  ...
  it('youLiaoTianJiLu为true时隐藏"暂无聊天记录"提示', async () => {
    vi.mocked(huoQuJunShiZhiDaoZhuangTai).mockResolvedValue({
      zhuangTai: null,
      keZaiCiZhiDao: false,
      youLiaoTianJiLu: true,
  ...
  it('keZaiCiZhiDao为false且非指导中时显示"已指导过相同内容"提示', async () => {
    vi.mocked(huoQuJunShiZhiDaoZhuangTai).mockResolvedValue({
      zhuangTai: null,
      keZaiCiZhiDao: false,
      youLiaoTianJiLu: true,
  ...
  it('keZaiCiZhiDao为true时隐藏"已指导过相同内容"提示', async () => {
    vi.mocked(huoQuJunShiZhiDaoZhuangTai).mockResolvedValue({
      zhuangTai: null,
      keZaiCiZhiDao: true,
      youLiaoTianJiLu: true,
  ...
  it('已完成状态且keZaiCiZhiDao为false时显示"已指导过相同内容"提示', async () => {
    vi.mocked(huoQuJunShiZhiDaoZhuangTai).mockResolvedValue({
      zhuangTai: {
        zhuang_tai: 'yi_wan_cheng',
        jun_shi_id: 'xuanRuiMu',
...
describe('FP-11 军师指导分区协议在面板内的渲染', () => {
  const fenDuan = {
  ...
  async function zhanKaiXuanRuiMuJieGuo(wrapper: ReturnType<typeof mount>) {
    const kapian = huoQuJunShiKapianByJunShiId(wrapper, 'xuanRuiMu')
    await kapian?.find('.qingqiu-anniu').trigger('click')
    await flushPromises()
    return kapian
  }

  it('请求返回分段：分区渲染且顺序为「下一步怎么回」最前', async () => {
    vi.mocked(qingQiuJunShiZhiDao).mockResolvedValue({
      junShi: chuangJianMoNiJunShiLieBiao()[0],
      zhiDaoNeiRong: zhengDuan,
    ...
    expect(wrapper.find('.zhidao-jieguo .jieguo-neirong').exists()).toBe(false)
    expect(wrapper.findAll('.zhidao-jieguo .junshi-duan').map((duan) => duan.attributes('data-duan')))
      .toEqual(['xiaYiBuZenMeHui', 'dangQianJuMian', 'weiShenMeZheMeLiao', 'guLi'])
    expect(wrapper.find('.zhidao-jieguo .zhidian-neirong').text()).toBe(fenDuan.xiaYiBuZenMeHui)
    expect(wrapper.find('[data-duan="dangQianJuMian"] .duan-neirong').text()).toBe(
      fenDuan.dangQianJuMian,
    )
  })

  it('结构化后仍保留 AI 生成标识', async () => {
    vi.mocked(qingQiuJunShiZhiDao).mockResolvedValue({
      junShi: chuangJianMoNiJunShiLieBiao()[0],
      zhiDaoNeiRong: zhengDuan,
      zhiDaoFenDuan: fenDuan,
  ...
  it('面板内「复制这句回复」复制成品话术而非整段', async () => {
    ...
    const anNiu = wrapper.find('.zhidao-jieguo .fuzhi-anniu')
    expect(anNiu.text()).toBe(huoQuFanYi('junShi', 'fuZhiZheJuHua'))
    await anNiu.trigger('click')
    await vi.waitFor(() => expect(xieRu).toHaveBeenCalledWith(fenDuan.xiaYiBuZenMeHui))
    expect(xieRu.mock.calls[0][0]).not.toBe(zhengDuan)
    expect(wrapper.find('.zhidao-jieguo .fuzhi-tishi').text()).toBe(huoQuFanYi('junShi', 'yiFuZhi'))
  })

  it('轮询到已完成状态带分段时同样分区渲染', async () => {
    vi.mocked(huoQuJunShiZhiDaoZhuangTai).mockResolvedValue({
      zhuangTai: {
        zhuang_tai: 'yi_wan_cheng',
        jun_shi_id: 'xuanRuiMu',
  ...
  it('旧记录只有整段文本时面板内整段兜底且不出现复制按钮', async () => {
    vi.mocked(huoQuJunShiJiLu).mockResolvedValue([
      {
        jian_yi: '老版本整段建议，一段到底。',
        shi_jian: '2026-07-07T10:00:00.000Z',
        jiao_se_id: 'j1',
        jiao_se_ming_zi: '小甜心',
        jun_shi_id: 'xuanRuiMu',
    ...
    expect(wrapper.find('.zhidao-jieguo .jieguo-neirong').text()).toBe('老版本整段建议，一段到底。')
    expect(wrapper.find('.zhidao-jieguo .junshi-fenduan').exists()).toBe(false)
    expect(wrapper.find('.zhidao-jieguo .fuzhi-anniu').exists()).toBe(false)
  })

  it('指导记录列表预览优先取成品话术，旧记录仍取整段', async () => {
    const chaoChangHuaShu = '话'.repeat(60)
    const chaoChangZhengDuan = `局面${'局'.repeat(60)}\n${chaoChangHuaShu}`
    vi.mocked(huoQuJunShiJiLu).mockResolvedValue([
      {
        jian_yi: chaoChangZhengDuan,
        jian_yi_fen_duan: { ...fenDuan, xiaYiBuZenMeHui: chaoChangHuaShu },
        shi_jian: '2026-07-07T10:00:00.000Z',
        jiao_se_id: 'j1',
        jiao_se_ming_zi: '小甜心',
        jun_shi_id: 'xuanRuiMu',
    ...
    const yuLanLieBiao = wrapper.findAll('.jilu-yulan')
    expect(yuLanLieBiao).toHaveLength(2)
    expect(yuLanLieBiao[0].text()).toBe(`${'话'.repeat(50)}...`)
    expect(yuLanLieBiao[0].text()).not.toContain('局面')
    expect(yuLanLieBiao[1].text()).toBe(`${chaoChangZhengDuan.slice(0, 50)}...`)
    expect(yuLanLieBiao[1].text()).toContain('局面')
  })
})
