...
describe('P1-11 撤回窗口时间刷新策略', () => {
  ...
  it('FP-09b 走真实发送链路进来的用户消息同样按到期时刻翻转撤回按钮', async () => {
    ...
    vi.mocked(faSongXiaoXi).mockImplementation(async ({ neiRong, miDengJian }) => ({
      xiaoXi: {
        ...
        ke_hu_duan_xu_hao: 12,
      },
      shiMiJi: false,
    }))

    await 聊天仓库.faSongXiaoXi('真实链路进来的可撤回消息')
    await flushPromises()

    expect(聊天仓库.xiaoXiLieBiao[0].id).toBe('luo-ku-chehui')
    expect(聊天仓库.xiaoXiLieBiao[0].mi_deng_jian).toMatch(
