...
describe('过往战绩排序', () => {
  ...
  describe('FP-04 拖拽落点权威与分页索引空间', () => {
    // 造一个带 N 张 .zhanji-kapian 子节点的容器，满足 start 的中心捕获与 ghost 查询
    function zaoTuoRongQi(ids: string[]): HTMLElement {
      const rongQi = document.createElement('div')
      rongQi.className = 'zhanji-liebiao-neirong'
      for (const id of ids) {
        const ka = document.createElement('div')
        ka.className = 'zhanji-kapian'
        ka.setAttribute('data-id', id)
        rongQi.appendChild(ka)
      }
      document.body.appendChild(rongQi)
      return rongQi
    }

    it('指针推算生效后，落定以推算下标为准（事件索引被库写成原值也不回弹）', async () => {
      localStorage.setItem(
        'zhanJiPaiXu_yong-hu-1',
        JSON.stringify({ jinxingzhong: ['jia', 'yi', 'bing'], shengli: [], shibai: [] }),
      )
      ...
      const shiLi = wrapper.vm as unknown as {
        fenLeiZu: Record<string, DangAnXiangQing[]>
        onTuoZhuaiKaiShi: (z: string, e?: unknown) => void
        onTuoZhuaiJieShu: (z: string, e?: unknown) => void
      }
      const rongQi = zaoTuoRongQi(['jia', 'yi', 'bing'])
      shiLi.onTuoZhuaiKaiShi('jinxingzhong', { from: rongQi, oldDraggableIndex: 0 })
      // jsdom 里所有矩形都是 0：指针落在 y=5 时推算结果为「末位」，预览即按此重排
      window.dispatchEvent(new MouseEvent('pointermove', { clientY: 5 }))
      await flushPromises()
      expect(wrapper.findAll('.jiaose-mingcheng').map((jie) => jie.text())).toEqual([
        '波乙',
        '此丙',
        '阿甲',
      ])

      // force-fallback 下库常把 old/new 都报成起始位；落定必须仍跟随预览，不得回弹
      shiLi.onTuoZhuaiJieShu('jinxingzhong', {
        oldIndex: 0,
        newIndex: 0,
        oldDraggableIndex: 0,
        ...
        from: rongQi,
        to: rongQi,
        originalEvent: new MouseEvent('mouseup', { clientY: 5 }),
      })
      await flushPromises()
      expect(shiLi.fenLeiZu.jinxingzhong.map((x) => x.id)).toEqual(['yi', 'bing', 'jia'])
      const cunChu = JSON.parse(localStorage.getItem('zhanJiPaiXu_yong-hu-1') || '{}')
    ...
    it('第二页拖拽只重排该页区间，其余页记录不丢', async () => {
      const quanBu: DangAnXiangQing[] = Array.from({ length: 55 }, (_, i) =>
        zaoDangAn({ id: `dang-${String(i).padStart(2, '0')}`, jiao_se_ming_zi: `角色${i}` }),
      )
      localStorage.setItem(
        'zhanJiPaiXu_yong-hu-1',
        JSON.stringify({ jinxingzhong: quanBu.map((x) => x.id), shengli: [], shibai: [] }),
      )
      const wrapper = await guaZai(quanBu)
      expect(wrapper.findAll('.zhanji-kapian').length).toBe(50)

      const fenZu0 = wrapper.findAll('.zhanji-fenlei-zu')[0]
      await fenZu0.findAll('.fen-ye-anniu')[1].trigger('click')
      await flushPromises()
      const ye2 = wrapper
        .findAll('.zhanji-fenlei-zu')[0]
        .findAll('.jiaose-mingcheng')
        .map((jie) => jie.text())
      expect(ye2).toEqual(['角色50', '角色51', '角色52', '角色53', '角色54'])
      ...
      // 页内把第 0 条拖到第 2 条：索引空间是页内 0..4，不是全量 50..54
      const rongQi = zaoTuoRongQi(['dang-50', 'dang-51', 'dang-52', 'dang-53', 'dang-54'])
      shiLi.onTuoZhuaiKaiShi('jinxingzhong', { from: rongQi, oldDraggableIndex: 0 })
      shiLi.onTuoZhuaiJieShu('jinxingzhong', {
        oldIndex: 0,
        newIndex: 2,
      ...
      const hou = shiLi.fenLeiZu.jinxingzhong.map((x) => x.id)
      expect(hou.length).toBe(55)
      expect(hou.slice(0, 50)).toEqual(quanBu.slice(0, 50).map((x) => x.id))
      expect(hou.slice(50)).toEqual([
        'dang-51',
        'dang-52',
        'dang-50',
        'dang-53',
    ...
    it('拖拽中其余分组仍按分页渲染，不会突然涨成全量', async () => {
      const lieBiao: DangAnXiangQing[] = [
        ...Array.from({ length: 52 }, (_, i) =>
          zaoDangAn({ id: `jin-${i}`, jiao_se_ming_zi: `进行中${i}` }),
        ),
        zaoDangAn({
          id: 'sheng-1',
          jiao_se_ming_zi: '胜1',
      ...
      const wrapper = await guaZai(lieBiao)
      expect(wrapper.findAll('.zhanji-fenlei-zu')[0].findAll('.zhanji-kapian').length).toBe(50)
      const shiLi = wrapper.vm as unknown as {
        onTuoZhuaiKaiShi: (z: string, e?: unknown) => void
        onTuoZhuaiJieShu: (z: string, e?: unknown) => void
      }
      const rongQi = zaoTuoRongQi(['jin-0', 'jin-1'])
      shiLi.onTuoZhuaiKaiShi('jinxingzhong', { from: rongQi, oldDraggableIndex: 0 })
      await flushPromises()
      expect(wrapper.findAll('.zhanji-fenlei-zu')[0].findAll('.zhanji-kapian').length).toBe(50)
      shiLi.onTuoZhuaiJieShu('jinxingzhong', {})
