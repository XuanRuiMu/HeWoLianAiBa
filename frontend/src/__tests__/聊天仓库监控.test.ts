...
// FP-08：订阅门的能力位来源必须是服务端出参，而不是测试手灌
vi.mock('@/api/认证', async (原模块) => ({
  ...(await 原模块<typeof import('@/api/认证')>()),
  huoQuYongHuXinXi: vi.fn(async (): Promise<Yonghu> => ({ id: 'u1' }) as unknown as Yonghu),
}))

const { jiaSocket, shiJianChuLiQi, zhuCeQingDan } = vi.hoisted(() => {
  const shiJianChuLiQi: Record<string, (shuJu: unknown) => void> = {}
  const zhuCeQingDan: string[] = []
  const jiaSocket = {
    on: (shiJian: string, chuLiQi: (shuJu: unknown) => void) => {
      shiJianChuLiQi[shiJian] = chuLiQi
      zhuCeQingDan.push(shiJian)
    },
    emit: vi.fn(),
    disconnect: vi.fn(),
    connected: false,
  }
  return { jiaSocket, shiJianChuLiQi, zhuCeQingDan }
})
...
const GUAN_LI_SHI_JIAN = ['管理员_构建过程', '管理员_好感度变化', '管理员_隐藏信息', '管理员_深度思考']
...
describe('聊天 store 管理员实时监控事件', () => {
  beforeEach(() => {
    ...
    zhuCeQingDan.length = 0
    vi.clearAllMocks()
    // FP-19 订阅门：管理员_* 监听器只为服务端下发的 cha_kan（只读运营数据）能力注册，用例先点亮身份再建 socket
...
describe('聊天 store 旧内心消息统一持久化补全', () => {
  beforeEach(() => {
    ...
    Object.keys(shiJianChuLiQi).forEach((jian) => delete shiJianChuLiQi[jian])
    vi.clearAllMocks()
    // FP-19 订阅门后，直接驱动 管理员_* 处理器的用例需先点亮 cha_kan 视图门
    使用用户仓库().nengLieBiao = ['cha_kan']
  })
...
describe('FP-19 管理员_* 前端订阅门（普通用户既收不到也不订阅）', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    localStorage.clear()
    Object.keys(shiJianChuLiQi).forEach((jian) => delete shiJianChuLiQi[jian])
  ...
  it('非管理员建连时零 管理员_* 监听注册，业务事件监听不受影响', () => {
    使用用户仓库().nengLieBiao = []
    const cangKu = 使用聊天仓库()
    cangKu.lianJieSocket('h1')

    for (const shiJian of GUAN_LI_SHI_JIAN) {
      expect(shiJianChuLiQi[shiJian]).toBeUndefined()
      expect(zhuCeQingDan).not.toContain(shiJian)
    }
    // 收口不得收过头：角色回复/AI状态/游戏事件 等业务监听恒在
    expect(shiJianChuLiQi['角色回复']).toBeTypeOf('function')
    expect(shiJianChuLiQi['AI状态']).toBeTypeOf('function')
    expect(shiJianChuLiQi['游戏事件']).toBeTypeOf('function')

    cangKu.duanKaiSocket()
  })

  it('管理员建连即订阅且幂等：重复 queBaoJianKongDingYue 不产生第二份监听', () => {
    使用用户仓库().nengLieBiao = ['cha_kan']
    const cangKu = 使用聊天仓库()
    cangKu.lianJieSocket('h1')
    cangKu.queBaoJianKongDingYue()
    cangKu.queBaoJianKongDingYue()

    for (const shiJian of GUAN_LI_SHI_JIAN) {
      expect(shiJianChuLiQi[shiJian]).toBeTypeOf('function')
      expect(zhuCeQingDan.filter((ming) => ming === shiJian)).toHaveLength(1)
    }

    cangKu.duanKaiSocket()
  })

  it('建连时身份未就绪（非管理员）、秘籍确认管理员后补挂监听（幂等入口）', () => {
    使用用户仓库().nengLieBiao = []
    const cangKu = 使用聊天仓库()
    cangKu.lianJieSocket('h1')
    expect(shiJianChuLiQi['管理员_构建过程']).toBeUndefined()

    // 模拟 greedisgood → queBaoShenFenJiuXu 确认管理员后补挂（聊天页面.queBaoJianKongDingYue 路径）
    使用用户仓库().nengLieBiao = ['cha_kan']
    cangKu.queBaoJianKongDingYue()
    expect(shiJianChuLiQi['管理员_构建过程']).toBeTypeOf('function')
    cangKu.queBaoJianKongDingYue()
    expect(zhuCeQingDan.filter((ming) => ming === '管理员_构建过程')).toHaveLength(1)

    cangKu.duanKaiSocket()
  })

  it('订阅门的能力位来自服务端出参真链路（huoQuYongHuXinXi → 归一管理能力列表），不靠测试手灌', async () => {
    const 用户仓库 = 使用用户仓库()
    // 抹掉 beforeEach 的手灌值：此后 keGuanLiZhiDu 翻真的唯一来源必须是下面这次真实拉取
    用户仓库.nengLieBiao = []
    用户仓库.令牌 = 'test-token'
  ...
  it('socket 重建（换端/放弃重连后新建）按当时身份重新注册，旧注册标志不残留', () => {
    使用用户仓库().nengLieBiao = ['cha_kan']
    const cangKu = 使用聊天仓库()
    cangKu.lianJieSocket('h1')
    cangKu.duanKaiSocket()
