...
describe('FP-01 草地背景配置与节流', () => {
  ...
  it('父帧三维库等待上限与轮询间隔随地址参数下发，空闲窗口数值可配置', () => {
    const diZhi = gouJianCaoDiDiZhi()
    expect(diZhi).toContain(`fuZhenLunXun=${caoDiPeiZhi.fuZhenSanWeiLunXunJianGeHaoMiao}`)
    expect(diZhi).toContain(`fuZhenChaoShi=${caoDiPeiZhi.fuZhenSanWeiChaoShiHaoMiao}`)
    expect(caoDiPeiZhi.fuZhenSanWeiChaoShiHaoMiao).toBeLessThan(30000)

    const fuGai = gouJianCaoDiDiZhi({
      fuZhenSanWeiLunXunJianGeHaoMiao: 30,
      fuZhenSanWeiChaoShiHaoMiao: 4000,
    })
    expect(fuGai).toContain('fuZhenLunXun=30')
    expect(fuGai).toContain('fuZhenChaoShi=4000')

    expect(caoDiPeiZhi.beiJingKongXianChaoShiHaoMiao).toBeGreaterThan(0)
    expect(caoDiPeiZhi.beiJingKongXianTuiJiHaoMiao).toBeGreaterThan(0)
  })
