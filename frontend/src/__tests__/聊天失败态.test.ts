...
describe('P1-7 聊天链路失败态治理', () => {
  ...
  describe('发送失败微信式感叹号与重发', () => {
    ...
    it('FP-09b 重发复用同一把幂等键，列表始终只有这一条用户消息', async () => {
      ...
      await 气泡[气泡.length - 1].find('.fasong-shibai-jiaobiao').trigger('click')
      await flushPromises()

      expect(vi.mocked(faSongXiaoXi)).toHaveBeenCalledTimes(2)
      expect(上报的键[0]).toMatch(UUID)
      expect(上报的键[1]).toBe(上报的键[0])
      // 重发不得产生第二条：原位同一条气泡换成落库行
      expect(聊天仓库.xiaoXiLieBiao.filter((m) => m.nei_rong === '重发这一句')).toHaveLength(1)
      expect(聊天仓库.xiaoXiLieBiao).toHaveLength(1)
      expect(聊天仓库.xiaoXiLieBiao[0].id).toBe('luo-ku-hang')
      expect(聊天仓库.xiaoXiLieBiao[0].mi_deng_jian).toBe(上报的键[0])
