...
J = {
    ...
    "wriL":     ((0.062, -0.352, 0.062), 0.029, 0.027),
    "hndL":     ((0.045, -0.388, 0.072), 0.028, 0.021),
    "shoR":     ((-0.098, -0.185, 0.086), 0.045, 0.045),
    "elbR":     ((-0.150, -0.268, 0.040), 0.038, 0.038),
    "wriR":     ((-0.062, -0.352, 0.062), 0.029, 0.027),
    "hndR":     ((-0.045, -0.388, 0.072), 0.028, 0.021),
...
for s in (1, -1):
    加球("眼%d" % s, (0.022*s, -0.373, HZ+0.044), 0.0075, C_头发, sx=0.8, sy=0.5, sz=1.0)
...
for s in (1, -1):
    加框("镜圈%d" % s, (0.022*s, -0.3785, HZ+0.044), 0.021, 0.0028, 0.013, C_镜框)
...
加框("鼻梁", (0, -0.380, HZ+0.047), 0.010, 0.0023, 0.0020, C_镜框)

# ---------- 手机（归位双手之间：hndL/R 中点前下）----------
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, -0.412, 0.062))
机 = bpy.context.active_object; 机.name = "手机"
机.scale = (0.034, 0.005, 0.068)
机.rotation_euler = (math.radians(80), 0, 0)
...
def 渲染(名, 位, 目标, res=900):
    ...
    sc.render.filepath = os.path.join(OUT, "FP-10-v7-%s.png" % 名)
...
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(DIR, "吴昊阳模型v7-重做.blend"))
print("FP10] DONE")
