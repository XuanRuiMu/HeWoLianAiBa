# -*- coding: utf-8 -*-
"""FP-10 重做 v2：修复 v1 读图自查发现的6个问题。
1) Workbench 开 MATERIAL 色显示（v1 全灰）
2) 手机下移到双手之间并立起（v1 屏幕糊脸像面具）
3) 双手向中线收拢贴住手机
4) 头/发帽增大（v1 头身比失调）
5) 腿分开、加长、贴地（v1 屁股拱起腿悬空）
6) 输出 v2 文件名
"""
...
# ---------- 身体骨架（趴姿：胸朝下，背朝上，头朝-Y，脚朝+Y）----------
J = {
    "pelvis":   ((0.000,  0.020, 0.125), 0.080, 0.062),
    "waist":    ((0.000, -0.060, 0.122), 0.064, 0.054),
    "chest":    ((0.000, -0.150, 0.130), 0.088, 0.070),
    "neck":     ((0.000, -0.235, 0.148), 0.034, 0.034),
    "headb":    ((0.000, -0.268, 0.156), 0.046, 0.046),
    "shoL":     ((0.100, -0.200, 0.148), 0.047, 0.047),
    "elbL":     ((0.150, -0.272, 0.075), 0.039, 0.039),
    "wriL":     ((0.052, -0.360, 0.105), 0.030, 0.028),
    "hndL":     ((0.040, -0.390, 0.118), 0.030, 0.022),
    "shoR":     ((-0.100, -0.200, 0.148), 0.047, 0.047),
    "elbR":     ((-0.150, -0.272, 0.075), 0.039, 0.039),
    "wriR":     ((-0.052, -0.360, 0.105), 0.030, 0.028),
    "hndR":     ((-0.040, -0.390, 0.118), 0.030, 0.022),
    "hipL":     ((0.078,  0.060, 0.110), 0.058, 0.058),
    "kneL":     ((0.115,  0.270, 0.070), 0.048, 0.048),
    "ankL":     ((0.130,  0.480, 0.050), 0.036, 0.036),
    "toeL":     ((0.138,  0.575, 0.022), 0.034, 0.020),
    "hipR":     ((-0.078,  0.060, 0.110), 0.058, 0.058),
    "kneR":     ((-0.115,  0.270, 0.070), 0.048, 0.048),
    "ankR":     ((-0.130,  0.480, 0.050), 0.036, 0.036),
    "toeR":     ((-0.138,  0.575, 0.022), 0.034, 0.020),
}
...
def 区域(p):
    ...
    if y < -0.24 and z > 0.16 and ax < 0.06: return 0  # 颈以上 → 皮肤
    return 1                        # 躯干 → 球衣
...
# ---------- 头（放大修复）----------
头 = 加球("头", (0, -0.310, 0.165), 0.068, C_皮肤, sx=0.94, sy=1.08, sz=1.04)
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, -0.330, 0.138))
...
bpy.ops.mesh.primitive_cone_add(vertices=16, radius1=0.010, radius2=0.004, depth=0.020,
                                location=(0, -0.382, 0.160), rotation=(math.radians(-100), 0, 0))
鼻 = bpy.context.active_object; 鼻.name = "鼻"; 鼻.data.materials.append(C_皮肤); bpy.ops.object.shade_smooth()
for s in (1, -1):
    加球("耳%d" % s, (0.062*s, -0.300, 0.162), 0.014, C_皮肤, sx=0.5, sy=0.9, sz=1.1)
for s in (1, -1):
    加球("眼%d" % s, (0.022*s, -0.376, 0.168), 0.008, C_头发, sx=0.8, sy=0.5, sz=1.0)

# ---------- 头发（帽壳+刺，随头放大）----------
bpy.ops.mesh.primitive_uv_sphere_add(segments=32, ring_count=16, radius=0.073, location=(0, -0.307, 0.177))
...
random.seed(7)
for i in range(26):
    ang = random.uniform(0, math.pi*2); rr = random.uniform(0.02, 0.058)
    px = rr*math.cos(ang)*0.92
    py = -0.307 + rr*math.sin(ang)*1.02
    if py > -0.275 and abs(px) < 0.05:
        continue
    pz = 0.222 + random.uniform(0.0, 0.02)
...
for s in (1, -1):
    加框("镜圈%d" % s, (0.022*s, -0.381, 0.168), 0.031, 0.021, 0.004, C_镜框)
    加球("镜片%d" % s, (0.022*s, -0.3825, 0.168), 0.012, C_镜片, sx=1.2, sy=0.25, sz=0.8)
    bpy.ops.mesh.primitive_cylinder_add(vertices=8, radius=0.0018, depth=0.075,
        location=(0.053*s, -0.345, 0.170), rotation=(math.radians(80), 0, math.radians(8*s)))
    腿 = bpy.context.active_object; 腿.name = "镜腿%d" % s
    腿.data.materials.append(C_镜框)
加框("鼻梁", (0, -0.383, 0.172), 0.014, 0.004, 0.003, C_镜框)

# ---------- 手机（下移到双手之间，立起微倾）----------
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, -0.408, 0.118))
机 = bpy.context.active_object; 机.name = "手机"
机.scale = (0.037, 0.006, 0.075)
机.rotation_euler = (math.radians(72), 0, 0)
机.data.materials.append(C_手机)
屏 = 加球("屏幕", (0, -0.414, 0.1185), 0.030, 颜色(0.25, 0.35, 0.5, 0.2), sx=1.12, sy=0.10, sz=2.2)
屏.rotation_euler = (math.radians(72), 0, 0)
...
def 渲染(名, 位, 目标, res=900):
    ...
    if bg: bg.inputs[0].default_value = (0.85, 0.9, 0.95, 1)
    sc.render.engine = 'BLENDER_WORKBENCH'
    sc.display.shading.light = 'STUDIO'
    sc.display.shading.color_type = 'MATERIAL'
    sc.display.shading.show_shadows = True
    sc.render.resolution_x = res; sc.render.resolution_y = res
    sc.render.filepath = os.path.join(OUT, "FP-10-v2-%s.png" % 名)
...
渲染("顶", (0, -0.10, 1.5), (0, -0.10, 0.10))

bpy.ops.wm.save_as_mainfile(filepath=os.path.join(DIR, "吴昊阳模型v5b-重做.blend"))
print("FP10] DONE")
