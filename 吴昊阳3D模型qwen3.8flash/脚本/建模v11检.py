...
C_短裤 = 材质纯色("短裤", 0.09, 0.12, 0.21, 0.55)
C_白袜 = 材质纯色("袜子", 0.94, 0.94, 0.95, 0.7)
C_白鞋 = 材质纯色("鞋子", 0.96, 0.96, 0.96, 0.5)
...
J = {
    ...
    "hipL":     ((0.075,  0.055, 0.054), 0.056, 0.050),
    "kneL":     ((0.135,  0.250, 0.038), 0.047, 0.040),
    "ankL":     ((0.175,  0.450, 0.030), 0.035, 0.030),
    "toeL":     ((0.195,  0.545, 0.012), 0.033, 0.014),
    "hipR":     ((-0.075,  0.055, 0.054), 0.056, 0.050),
    "kneR":     ((-0.135,  0.250, 0.038), 0.047, 0.040),
    "ankR":     ((-0.175,  0.450, 0.030), 0.035, 0.030),
    "toeR":     ((-0.195,  0.545, 0.012), 0.033, 0.014),
}
...
for side in (1, -1):
    ...
    # 短裤：粉横条 + 青下摆（膝上方）
    腿环("短裤粉条%d" % side, hip, kne, 0.80, 0.046, 0.006, C_粉线)
    腿环("短裤青摆%d" % side, hip, kne, 0.92, 0.044, 0.006, C_青边)
    # 短裤侧红菱（菱形片贴大腿外侧）
    ...
    # 袜：青顶边（踝上方）
    腿环("袜青边%d" % side, kne, ank, 0.10, 0.035, 0.005, C_青边)
...
def 渲染(名, 位, 目标, res=900):
    ...
    sc.render.filepath = os.path.join(OUT, "FP-10-v10-%s.png" % 名)
...
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(DIR, "吴昊阳模型v11检-临时.blend"))
print("FP10] DONE")
