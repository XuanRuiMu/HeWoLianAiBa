...
const info = await frame.evaluate(() => {
  const zu = window.__wu3dZu; const exp = window.__experience;
  const cam = exp.engine.camera.instance || exp.engine.camera;
  const out = { daoLu: window.__wu3DaoLu, hasZu: !!zu };
  if (zu) {
    zu.updateMatrixWorld(true);
    const box = new (Object.getPrototypeOf(zu).constructor === Object ? window.parent.THREE : window.parent.THREE).Box3().setFromObject(zu);
    out.boxMin = box.min.toArray().map(v => +v.toFixed(2));
    out.boxMax = box.max.toArray().map(v => +v.toFixed(2));
    const c = box.getCenter(new window.parent.THREE.Vector3()).project(cam);
    out.ndc = [+c.x.toFixed(2), +c.y.toFixed(2)];
    out.visible = zu.visible;
    let inScene = false; exp.engine.scene.traverse(o => { if (o === zu) inScene = true; });
    out.inScene = inScene;
  }
  return out;
});
await page.screenshot({ path: "证据/FP-03-真实页2.png" });
