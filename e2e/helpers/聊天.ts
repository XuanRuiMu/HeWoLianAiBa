...
export async function qingQiuJunShiZhiDao(page: Page): Promise<string> {
  ...
  // FP-11：新记录分区渲染（.junshi-fenduan），旧记录整段兜底（.jieguo-neirong），两者都必须有正文
  const jieGuoQu = page
    .locator('.zhidao-jieguo .junshi-fenduan, .zhidao-jieguo .jieguo-neirong')
    .first()
...
export async function qieHuanDaoJunShiJiLu(page: Page): Promise<void> {
  await page.locator('.zhidao-jilu-anniu').click()
  await expect(page.locator('.zhidao-jilu-mianban')).toBeVisible()
}
